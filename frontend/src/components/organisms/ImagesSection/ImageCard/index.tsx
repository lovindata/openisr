...
interface Props {
  ...
  onLatestProcessQueryShow?: (
    _: components["schemas"]["ProcessODto"] | undefined
  ) => boolean;
}

export function ImageCard({ image, onLatestProcessQueryShow }: Props) {
  ...
  const isShown =
    (onLatestProcessQueryShow && onLatestProcessQueryShow(latestProcess)) ??
    true;
  return (
    isShown && (
